(function() {
    ...
    handwriting.recognize = function(trace, options, callback) {
        if (handwriting.Canvas && this instanceof handwriting.Canvas) {
          ...
          var lang = "universal"
            if(  $(".btn-draw-text").hasClass("active")  ){
                lang = "en"
            }  
        ...
        if( $(parent.document).find(".btn-draw-text").hasClass("active") ){
            xhr.open("POST", "https://www.google.com.tw/inputtools/request?ime=handwriting&app=mobilesearch&cs=1&oe=UTF-8");
        }else{
            console.log("its a shape reco!")
            xhr.open("POST", "https://inputtools.google.com/request?itc=und-t-i0-handwrit&app=hwtcharpicker");
